...
*  +---+---+---+---+---+---+---+----------+----------+------------------+---------------+
*  Instruction length is anywhere between 1 to 17 bytes, I am not 100% certain why 15 is choose in
*  sandsifter. I am assuming the prefix length is a max of 2 for this choice, but still uncertain
*  
*/
...
static const insn_t null_insn = {}; // NUL insn. 

CONTEXT fault_context;

/* feedback */
typedef enum { TEXT, RAW } output_t; // RAW breaks, it causes a _CrtIsValidHeapPointer assertion failure, or something
...
// This might need to be edited for windows
...
void inject(int insn_size)
{
    ...
    if (config.enable_null_access) {
        void* p = NULL;
        ZeroMemory(p, PAGE_SIZE); // change this to something that isn't memset
    }
...
#else
    // Set general-purpose registers to the saved state.
    _mm_setcsr(injector_state.eax);
    _mm_setcsr(injector_state.ebx);
    _mm_setcsr(injector_state.ecx);
    _mm_setcsr(injector_state.edx);
    _mm_setcsr(injector_state.esi);
    _mm_setcsr(injector_state.edi);
    _mm_setcsr(injector_state.ebp);
    _mm_setcsr(injector_state.esp);
...
void give_result(FILE* f)
{
    ...
    case TEXT:
        ...
        case DRIVEN:
            ...
            if (result.exceptionCode == EXCEPTION_ILLEGAL_INSTRUCTION) { sync_fprintf(f, "illegal instr"); }        // SIGILL
            if (result.exceptionCode == EXCEPTION_INT_DIVIDE_BY_ZERO) { sync_fprintf(f, "int div by zero"); }       // SIGFPE
            if (result.exceptionCode == EXCEPTION_DATATYPE_MISALIGNMENT) { sync_fprintf(f, "data misalign"); }      // SIGBUS
            if (result.exceptionCode == EXCEPTION_SINGLE_STEP) { sync_fprintf(f, "single step"); }                  // SIGTRAP
            sync_fprintf(f, " %2d ", result.exceptionFlags);                                                        // si_num 
            sync_fprintf(f, " %02x ", result.faultingAddress);                                                      // addr
...
#else
    uintptr_t insn_length = (uintptr_t)context->Eip - (uintptr_t)packet - preamble_length;
#endif
...
int main(int argc, char** argv) {
    ...
    packet_buffer_unaligned = VirtualAlloc(NULL, PAGE_SIZE*3, MEM_RESERVE | MEM_PHYSICAL | MEM_LARGE_PAGES, PAGE_EXECUTE_READWRITE);
    if (packet_buffer_unaligned != NULL) 
    {
        ZeroMemory(&packet_buffer_unaligned, sizeof(packet_buffer_unaligned));
    }
